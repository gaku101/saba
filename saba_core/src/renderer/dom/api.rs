...
pub fn get_element_by_id(node: Option<Rc<RefCell<Node>>>, id_name: &String) -> Option<Rc<RefCell<Node>>> {
  match node {
    Some(n) => {
...
pub fn get_target_element_node(node: Option<Rc<RefCell<Node>>>, element_kind: ElementKind) -> Option<Rc<RefCell<Node>>> {
  match node {
    Some(n) => {
